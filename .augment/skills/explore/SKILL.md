...
Stop here. Never treat the original user request as permission to implement. Only a reply to the final implementation-path question can authorize implementation. Do not launch osf-apply, osf-proposal, osf-verify, autopilot, or any implementation subagent before that choice.
...
When user chooses Autopilot → use the Skill tool to invoke `autopilot` with the locked requirement summary and implementation review plan. Do not manually chain osf-proposal, osf-apply, or osf-verify from explore mode.
