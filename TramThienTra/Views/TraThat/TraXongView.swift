...
struct TraXongView: View {
            ...
            TimelineView(.animation) { timeline in
                Canvas { context, size in
                    ...
                    context.fill(
                        ...
                        with: .color(ZenColor.zenBrownDark)
                    )
                    ...
                    for i in 0..<particleCount {
                        ...
                        // Gold glow layer — pulsing opacity applied via @State glowOpacity
                        // The glow is drawn as a slightly larger ellipse behind the smoke
                        if !reduceMotion {
                            let glowRadius = radius * 1.3
                            let glowRect = CGRect(
                                x: smokePos.x - glowRadius,
                                y: smokePos.y - glowRadius,
                                width: glowRadius * 2,
                                height: glowRadius * 2
                            )
    ...
    // MARK: - Gold glow pulsing animation: 0.3 → 0.5 → 0.3 over 2s ease-in-out repeating
