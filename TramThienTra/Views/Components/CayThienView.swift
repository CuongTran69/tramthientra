...
//
// Animation behavior (Task 10.2):
//   • On stage change: spring bounce (scale 1.0 → 1.08 → 1.0,
//     response 0.4, dampingFraction 0.55).
//   • Reduce Motion: opacity crossfade only, no scaling.
//   • First appearance does not bounce — it simply sets the stage.
//
// Token usage (Task 12.1):
//   • No inline Color(hex:) — all colors resolve via ZenColor.
//   • No Color.zenAccent — migrated to ZenColor.zenSage.
...
struct CayThienView: View {
    ...
    var body: some View {
        HStack(spacing: 12) {
            // Stage visualization — bounces only on stage change, not on first appear
            ZStack {
